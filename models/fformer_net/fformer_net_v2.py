...
class base_DecoderV2(nn.Module):
    def __init__(self, d_model, nheads):
        ...
        self.distributor = nn.Conv2d(d_model + 1, d_model, kernel_size=3, padding=1)
    ...
    def forward(self, F_t: torch.Tensor, F_r: torch.Tensor, m_t: torch.Tensor, m_r: torch.Tensor):
        """
        Args:
            F_t (torch.Tensor): current frame feature
            F_r (torch.Tensor): reference frame feature
            m_t (torch.Tensor): mask for current frame from deeper decoder block b c h w
            m_r (torch.Tensor): mask for reference frame, b 1 h w
        """
        b, c, h, w = F_t.shape
        mask = self.up_sample(m_r, size=(h, w))
        F_r = self.distributor(torch.cat([F_r, mask], dim=1))
        ...
        out = torch.cat([F_t, rev_out], dim=1)
        out = self.conv_out(out)
        
        return out


class Former_DecoderV2(nn.Module):
    def __init__(self, ):
        super().__init__()
        self.decoder_2 = base_DecoderV2(d_model=32, nheads=8)
        self.decoder_3 = base_DecoderV2(d_model=32, nheads=8)
        self.decoder_4 = base_DecoderV2(d_model=32, nheads=8)
    
    def forward(self, x2_t, x3_t, x4_t, m5_t, x2_r, x3_r, x4_r, mask):
        m4_t = self.decoder_4(x4_t, x4_r, m5_t, mask)
        m3_t = self.decoder_3(x3_t, x3_r, m4_t, mask)
        m2_t = self.decoder_2(x2_t, x2_r, m3_t, mask)

        return m2_t, m3_t, m4_t


class FFormer_NetV2(nn.Module):
    """
    branch 2
    ...
    def __init__(self,):
        ...
        self.decoder = Former_DecoderV2()
        self.up_sample = lambda x, size: F.interpolate(x, size=size, mode='bilinear', align_corners=True)
    
    def forward(self, Fs, mask):
        b, T, c, h, w = Fs.shape
        with torch.no_grad():
        ...
        for t in range(1, T):
            ...
            m2_t, m3_t, m4_t = self.decoder(x2_t, x3_t, x4_t, m5_t, x2_r, x3_r, x4_r, mask)
...
if __name__ == '__main__':
    ...
    seg_net = Seg_Net()
    fformer_net = FFormer_NetV2()
    fformer_net.encoder = copy.deepcopy(seg_net.encoder)
    
    x = torch.randn(2, 4, 3, 128, 128)
    mask = torch.randn(2, 4, 1, 128, 128)
    m2_out, m3_out, m4_out = fformer_net(x, mask[:, 0])
